...
export const listActiveJobs = async (
  ...
  try {
    ...
    const userApplications: mongoose.Types.ObjectId[] =
    ...
    const jobs: IJob[] = await Job.find(filterCriteria).populate({
      path: "userId",
  ...
  try {
    ...
    const applications = await JobApplication.find({ jobId: { $in: jobIds } })
      .populate({
        path: "applicantId",
        select:
          "username profileImageUrl profile.fullname profile.designation companyProfile.companyName",
      })
      .populate("jobId")
      .exec();

    const jobSpecificApplications = await JobApplication.find({
...
export const employerApplications = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const cancelJobApplication = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Application Canceled", applications });
  } catch (error) {
    console.error("Error fetching employee applications:", error);
    res.status(500).json({ success: false, message: "Internal server error" });
  }
...
export const userJobBlock = asyncHandler(
...
export const getFormSelectData = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(200)
      .json({ locations: distinctLocations, roles: distinctRoles });
  } catch (error) {
...
export const jobRoles = asyncHandler(async (req: Request, res: Response) => {
  try {
    const jobCategories = await JobCategory.find({ isBlocked: false }).select(
      "jobCategory"
    );
    const jobRoles = jobCategories.map((category) => category.jobCategory);
